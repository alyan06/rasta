...
//             Download the four zips from https://nces.ed.gov/ipeds/datacenter/DataFiles.aspx
...
const usa = hd
  ...
  .map(r => {
    const a = adm.get(r.UNITID) ?? {}, e = ef.get(r.UNITID) ?? {}, c = ic.get(r.UNITID) ?? {};
...
const output = {
  ...
  usaColumns: ["id", "name", "city", "state", "website", "control", "undergraduates", "applicants", "admitted", "enrolled", "sat25", "sat75", "satSubmitters", "act25", "act75", "testPolicy", "tuitionOutOfState", "roomAndBoard", "locale", "hbcu", "sizeBand"],
  pakistan, usa,
};
...
console.log(`pakistan ${pakistan.length} (websites ${pakistan.filter(r => r[4]).length}) · usa ${usa.length} (acceptance data ${withRate}, SAT ranges ${withSat}, tuition ${usa.filter(r => r[16] !== null).length})`);
