...
import { ArrowRight, Check, FloppyDisk, PencilSimple, Plus, Trash, ArrowCounterClockwise } from '@phosphor-icons/react';
...
export default function ProfilePage({ data, update, navigate, notify, signedIn, openAccount }: WorkspaceProps) {
  ...
  type Section = 'school' | 'college' | 'tests';
  const [editing, setEditing] = useState<Record<Section, boolean>>({ school: false, college: false, tests: false });
  useEffect(() => { setDraft(migrateProfile(data.profile)); setFresh(false); setAttempted(false); }, [data.profile]);
  const hasSchool = draft.secondary === 'olevel' ? (draft.oLevels?.length ?? 0) > 0 : draft.ssc !== '';
  const hasCollege = (draft.higherSecondary ?? draft.curriculum) === 'alevel' ? (draft.aLevels?.length ?? 0) > 0 : draft.hssc !== '' || (draft.predictedHssc !== undefined && draft.predictedHssc !== '');
  const hasTests = draft.net !== '' || draft.nu !== '' || draft.sat !== '';
  const gradeChips = (entries: SubjectGrade[] | undefined) => <ul className="grade-chips">{(entries ?? []).filter(entry => entry.subject.trim()).map(entry => <li key={entry.id}><span>{entry.subject}</span><strong>{entry.grade || '—'}</strong>{entry.predicted && <small>predicted</small>}</li>)}</ul>;
  /** Saves the whole draft; returns false (and shows the errors) when it cannot. */
  function persist(): boolean {
    setAttempted(true);
    if (errors.length) { notify('Check the details before saving.'); return false; }
    ...
    setFresh(false); setAttempted(false);
    return true;
  }
  const done = (section: Section) => { if (persist()) { setEditing(previous => ({ ...previous, [section]: false })); notify('Saved.'); } };
  const sectionHeader = (section: Section, title: string, hint: string, has: boolean) => <div className="panel-header"><div><h2>{title}</h2><p className="muted">{hint}</p></div>{editing[section] ? <button className="button" type="button" onClick={() => done(section)}><Check size={17} /> Done</button> : <button className="button secondary" type="button" onClick={() => setEditing(previous => ({ ...previous, [section]: true }))}>{has ? <><PencilSimple size={16} /> Edit</> : <><Plus size={16} /> Add grades</>}</button>}</div>;
  ...
  function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (persist()) { setEditing({ school: false, college: false, tests: false }); notify('Your profile is saved.'); }
  }
  ...
  return <div className="stack profile-page">
    ...
    <form className="stack" onSubmit={save} noValidate>
      ...
      <section className="panel stack" data-tour="grades">{sectionHeader('school', 'School results', 'Matric or O Levels.', hasSchool)}
        ...
        : !hasSchool ? <p className="muted">No school results yet. Add your Matric percentage or your O Level grades.</p>
        : draft.secondary === 'olevel' ? <div className="grade-view"><p className="grade-view-title">O Levels{draft.examYear ? ` · finished ${draft.examYear}` : ''}</p>{gradeChips(draft.oLevels)}</div>
        : <div className="grade-view"><p className="grade-view-title">Matric / SSC</p><p className="grade-view-value">{draft.ssc}%</p></div>}
      </section>

      <section className="panel stack">{sectionHeader('college', 'College results', 'FSc / Intermediate or A Levels.', hasCollege)}
        {!editing.college && (!hasCollege ? <p className="muted">No college results yet. Add your FSc marks or your A Level grades — predicted grades are fine if you’re still waiting.</p>
        : (draft.higherSecondary ?? draft.curriculum) === 'alevel' ? <div className="grade-view"><p className="grade-view-title">A Levels · {draft.stage === 'awaiting' ? 'result awaited' : 'final result'}</p>{gradeChips(draft.aLevels)}</div>
        : <div className="grade-view"><p className="grade-view-title">FSc / Intermediate · {draft.stage === 'awaiting' ? 'result awaited' : 'final result'}{draft.mathematics ? ' · with Mathematics' : ''}</p><div className="grade-view-row">{draft.hssc !== '' && <span>{draft.stage === 'awaiting' ? 'Part 1' : 'Final'} <strong>{draft.hssc}%</strong></span>}{draft.stage === 'awaiting' && draft.predictedHssc !== undefined && draft.predictedHssc !== '' && <span>Predicted final <strong>{draft.predictedHssc}%</strong></span>}</div></div>)}
        {editing.college && <><div className="form-grid"><label className="field"><span>Which one are you doing?</span><select value={draft.higherSecondary ?? draft.curriculum} onChange={event => setDraft(previous => ({ ...previous, higherSecondary: event.target.value as Profile['higherSecondary'], curriculum: event.target.value as Profile['curriculum'], ...(event.target.value === 'alevel' && previous.aLevels === undefined ? { aLevels: [] } : {}) }))}><option value="fsc">FSc / FA / ICS / Intermediate</option><option value="alevel">A Levels</option></select></label><label className="field"><span>Have you got your final result?</span><select value={draft.stage} onChange={event => set('stage', event.target.value as Profile['stage'])}><option value="awaiting">Not yet — I’ll use predicted grades</option><option value="completed">Yes, I have my final result</option></select></label></div>
        ...
        {(hasSchool || hasCollege) && <div className="academic-preview"><h3>{summary.isEstimate ? 'Your percentages for planning' : 'Your saved percentages'}</h3><div className="academic-preview-values"><span>Matric-equivalent <strong>{summary.ssc === null ? 'More details needed' : `${summary.ssc.toFixed(2)}%`}</strong></span><span>FSc-equivalent <strong>{summary.hssc === null ? 'More details needed' : `${summary.hssc.toFixed(2)}%`}</strong></span></div>{summary.notes.length > 0 && <details><summary>How these numbers are worked out</summary><ul>{summary.notes.map(note => <li key={note}>{note}</li>)}</ul></details>}{(draft.secondary === 'olevel' || draft.higherSecondary === 'alevel') && <p className="muted">Your grades are converted the way IBCC does it, so you never have to enter an equivalence percentage yourself. Planning only: A* counts as 90 and only IBCC can issue the official certificate. <a href={IBCC_SOURCES[0].url} target="_blank" rel="noreferrer">IBCC rules ↗</a></p>}</div>}
      </section>

      <section className="panel stack">{sectionHeader('tests', 'Test scores', 'Only scores you actually have. Try future scores in the planner.', hasTests)}
        {editing.tests ? <div className="form-grid">{scoreField('net', 'NUST NET score', 'Out of 200.', 0, 200, '1')}{scoreField('nu', 'FAST NU test score (%)', 'Your percentage after negative marking.', 0, 100)}{scoreField('sat', 'SAT score', 'Total out of 1600, in steps of 10.', 400, 1600, '10')}</div>
        : !hasTests ? <p className="muted">No test scores yet. Add a NET, NU test or SAT score when you have one.</p>
        : <div className="grade-view"><div className="grade-view-row">{draft.net !== '' && <span>NUST NET <strong>{draft.net} / 200</strong></span>}{draft.nu !== '' && <span>FAST NU test <strong>{draft.nu}%</strong></span>}{draft.sat !== '' && <span>SAT <strong>{draft.sat}</strong></span>}</div></div>}
        <button className="text-button" type="button" onClick={() => navigate('planner')}>Plan a target score <ArrowRight size={15} /></button></section>
