...
| AI help | Two buttons, no prompt box: **Improve with AI** rewrites an activity description in Common App style (≤150 characters), **Polish with AI** tidies an essay draft in the student’s own voice. Suggestions appear in violet with Keep / Revert. A Supabase Edge Function holds the fixed prompts and guardrails; free with daily quotas. See [docs/AI_SETUP.md](docs/AI_SETUP.md). |
...
**How activities count.** No AI is involved. `activityStrength()` in `src/lib/chancing.ts` scores the activity list from 0 to 1 in the browser: each entry earns a base amount, more for time commitment (hours per week × weeks per year), a substantive description (40+ characters — Common App allows 150), a leadership, research or work role, and a small bonus for recognition words such as *national* or *award*. Around 4–5 deep entries reach the top of the scale; ten one-line entries do not. That score nudges the chance up or down more at selective universities. Activity fields follow Common App character limits (role 50, organisation 100, description 150).
